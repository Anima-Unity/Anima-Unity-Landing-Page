...
export default function Navbar() {
  ...
  const isMinimal = ['/privacy-policy', '/terms-of-service', '/cookie-policy'].includes(pathname)
  ...
  return (
    <nav className="fixed top-0 w-full z-50 backdrop-blur-md bg-white/30 border-b border-white/20 transition">
      ...
      {navItems.map((item) => (
        <Link
          ...
          className={`block text-sm font-medium ${
            activeSection === item ? 'text-orange-500 font-semibold' : 'text-gray-800 hover:text-orange-500'
          }`}
        >
          {item.charAt(0).toUpperCase() + item.slice(1)}
        </Link>
